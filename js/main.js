window.onload = function(){
	...
	;(function(window) {
		...
		var body = document.body,
			...
			switchGridCtrls = [].slice.call(document.querySelectorAll('.ified a')),
		...
		function init() {
			...
			imagesLoaded(body, function() {
				...
				switchGridCtrls[0].classList.add('active');
			});
		}
		...
		function initEvents() {
			...
			switchGridCtrls.forEach(function(ctrl) {
				ctrl.addEventListener('click', function(ev){
					...
					for(var i=0;i<switchGridCtrls.length;i++){
						switchGridCtrls[i].classList.remove('active');
					}

					var _index = switchGridCtrls.indexOf(this);
					...
					var _as = document.querySelectorAll('.grid__link');

					for(var i=0;i<_as.length;i++){
						_as[i].style.opacity = 0;
					}
					// Show new grid.
					grids[currentGrid].classList.remove('grid--hidden');
